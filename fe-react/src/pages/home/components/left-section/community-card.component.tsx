...
const renderLiItems = (): ReactElement[] =>
  links.map(({ key, text, to }) => {
...
const CardBody: FC = () => (
  <main className="hu-community-card-main tw-p-3 tw-py-4">
    <ul className="tw-flex tw-flex-col tw-gap-5">{renderLiItems()}</ul>
  </main>
);

const CardFooter: FC = () => (
  <footer className="hu-community-card-footer tw-p-3 tw-text-center">
    <Link to="/discover" className="tw-text-sm tw-text-gray-600 tw-font-medium">
      Discover More
    </Link>
  </footer>
);
